...
namespace Typical_Tool {
	namespace StringManage
	{
		...
		bool WriteFile(const Tstr& _ConfigFilePath, std::vector<Tstr>& _WriteText, const Tstr& _FileEncode = _T("UTF-8"))
		{
			Tofstream WriteFileStream(_ConfigFilePath, std::ios::out);

			if (!WriteFileStream) {
				lgc(_T("WriteFile: [") + _ConfigFilePath + _T("] 打开文件失败!"), wr);
				return false;
			}
			lgc(_T("WriteFile: [") + _ConfigFilePath + _T("] 打开文件成功!"), ts);

			if (_FileEncode == _T("UTF-8BOM")) {
				// 写入 UTF-8 BOM (EF BB BF)
				lgc(_T("WriteFile: 写入 UTF-8BOM编码(EF BB BF)."), ts);

				WriteFileStream.put((Tchar)0xEF);
				WriteFileStream.put((Tchar)0xBB);
				WriteFileStream.put((Tchar)0xBF);
			}
			else if (_FileEncode == _T("UTF-8")) {
				lgc(_T("WriteFile: 写入 UTF-8编码."), ts);
			}

			for (const Tstr& tempStr : _WriteText) {
				WriteFileStream << tempStr;
			}
			lgc(_T("WriteFile: 写入完成!"), ts);
		...
		//读取文件
		template<class T = bool>
		bool ReadFile(const Tstr& _ConfigFilePath, std::vector<Tstr>& _ReadText, const Tstr& _FileEncode = _T("UTF-8"))
		{
			Tifstream ReadFileStream(_ConfigFilePath, std::ios::binary);

			if (!ReadFileStream) {
				lgc(_T("ReadFile: [") + _ConfigFilePath + _T("] 打开文件失败!"), wr);
				return false;
			}
			lgc(_T("ReadFile: [") + _ConfigFilePath + _T("] 打开文件成功!"), ts);

			// 读取 BOM（如果存在）
			if (_FileEncode == _T("UTF-8")) {
				...
				if (bom.size() == 3 && bom[0] == 0xEF && bom[1] == 0xBB && bom[2] == 0xBF) {
					lgc(_T("ReadFile: 检测到UTF-8 BOM."), ts);
				}
				else {
		...
		*/
		...
		public:
			...
			{
				lgc(_T("编码问题:"), ts);
				lgc(_T("Windows: 使用之前先统一编码[Unicode(UTF-8)]: 文件编码, 控制台/其它显示输出程序 编码, 源文件编码"), ts);
			}
		...
		public:
			...
			Tstr GetConfigItem_Value(const Tstr& _Config, const Tstr& _ConfigItem)
			{
				...
				if (tempConfig != this->ConfigMap.end()) {
					if (this->ShowManageLog) {
						lgc(_T("GetConfigItem_Value: 获取配置项 Value"));
						lgc(_T("  配置[") + _Config + _T("]"));
					}

					auto tempConfigItem = tempConfig->second.find(_ConfigItem);
					if (tempConfigItem != tempConfig->second.end()) {
						if (this->ShowManageLog) {
							lgc(_T("    配置项 Key  [") + _ConfigItem + _T("]"));
							lgc(_T("    配置项 Value[") + tempConfigItem->second + _T("]"));
						}
						return tempConfigItem->second;
					}
					else {
						lgc(_T("GetConfigItem_Value: 没有找到对应配置项!"), wr);
						return Tstr();
					}
				}
				else {
			...
			std::map<Tstr, Tstr> GetConfigItem(const Tstr& _Config, const Tstr& _ConfigItem)
			{
				...
				if (tempConfig != this->ConfigMap.end()) {
					...
					if (tempConfigItem != tempConfig->second.end()) {
						if (this->ShowManageLog) {
							lgc(_T("    配置项[") + _ConfigItem + _T("]"));
						}

						return { {tempConfigItem->first, tempConfigItem->second} };
					}
					else {
						lgc(_T("GetConfigItem: 没有找到对应配置项!"), wr);
			...
			{
				lgc(_T("OutConfigFile_All: 输出内容 ") + this->ConfigFilePath + _T(" 开始..."), ts);

				for (auto tempConfig = this->ConfigMap.begin(); tempConfig != this->ConfigMap.end(); tempConfig++) {
					lgc(_T("  配置[") + tempConfig->first + _T("]"));
					for (auto tempConfigText = tempConfig->second.begin(); tempConfigText != tempConfig->second.end(); tempConfigText++) {
						lgc(_T("    配置项 Key  : ") + tempConfigText->first);
						lgc(_T("    配置项 Value: ") + tempConfigText->second);
					}
				}
				lgc(_T("OutConfigFile_All: 输出内容 ") + this->ConfigFilePath + _T(" 结束!\n"), ts);
			}
			...
			*/
			...
			bool AddConfig(const Tstr& _Config, std::vector<Tstr>& _ConfigItem)
			{
				...
				for (auto tempConfigItem = _ConfigItem.begin(); tempConfigItem != _ConfigItem.end(); tempConfigItem++) {
					if (!this->ConfigItemTextManage(*tempConfigItem, tempAnalyzeLaterConfigItem_Key, tempAnalyzeLaterConfigItem_Value)) {
						lgc(_T("AddConfig: 配置项文本处理失败!"), wr);
						lgc(_T("  配置[") + _Config + _T("]"), wr);
						return false;
					}
					else { //成功处理后, 临时保存
				...
				if (tempConfig != this->ConfigMap.end()) { //重复配置
					if (this->ShowManageLog) {
						lgc(_T("AddConfig: 重复配置[") + _Config + _T("]添加配置项开始..."), ts);
					}
					...
					if (this->ShowManageLog) {
						lgc(_T("AddConfig: 配置[") + _Config + _T("]添加配置项开始..."), ts);
					}
				}
				...
				for (auto& ConfigItem : AnalyzeLaterConfigItemMap) {
					...
					if (this->ShowManageLog) {
						lgc(_T("    配置项 Key  [") + ConfigItem.first + _T("]"));
						lgc(_T("    配置项 Value[") + ConfigItem.second + _T("]"));
					}
				}
				...
				if (!tempPair.second) {
					lgc(_T("AddConfig: 配置[") + _Config + _T("]添加失败!"), wr);
					return false;
				}
				else {
					lgc(_T("AddConfig: 配置[") + _Config + _T("]添加成功!"), ts);
				}
			...
			bool DeleteConfigItem(const Tstr& _Config, const Tstr& _ConfigItem)
			{
				...
				if (temp != this->ConfigMap.end()) {
					if (temp->second.erase(_ConfigItem)) { //删除对应配置项
						...
						lgc(_T("DeleteConfigItem: 没有找到对应配置项!"), wr);
						return false;
					}
				}
				else {
					lgc(_T("DeleteConfigItem: 没有找到对应配置!"), wr);
					return false;
				}
			}
			...
			bool DeleteConfig(const Tstr& _Config, bool _IsDelete)
			{
				if (_IsDelete) {
					if (this->ConfigMap.erase(_Config)) {  //删除对应配置
						if (this->ShowManageLog) {
							lgc(_T("DeleteConfig: 删除配置"));
							lgc(_T("  配置[") + _Config + _T("]"));
						}
						return true;
					}
					else {
						lgc(_T("DeleteConfig: 没有找到对应配置!"), wr);
						return false;
					}
				}
				else {
					lgc(_T("DeleteConfig: 未进行二次确认[") + _Config + _T("]"), wr);
					return false;
				}
			}
			...
			bool SetConfigItemValue(const Tstr& _Config, const Tstr& _ConfigItem_Key, const Tstr& _ConfigItem_Value)
			{
				...
				if (tempConfig != this->ConfigMap.end()) {
					if (this->ShowManageLog) {
						lgc(_T("SetConfigItemValue: 修改配置项 Value"));
						lgc(_T("  配置[") + _Config + _T("]"));
					}
					auto tempConfigItem = tempConfig->second.find(_ConfigItem_Key); //对应配置
					if (tempConfigItem != tempConfig->second.end()) {
						if (this->ShowManageLog) {
							lgc(_T("    配置项 Key  [") + tempConfigItem->first + _T("]"));
							lgc(_T("    配置项 Value[") + tempConfigItem->second + _T("]"));
						}
						...
						if (this->ShowManageLog) {
							lgc(_T("SetConfigItemValue: 修改后..."), ts);
							lgc(_T("    配置项 Value[") + _ConfigItem_Value + _T("]"));
						}
					}
					...
					lgc(_T("SetConfigItemValue: 没有找到对应配置项!"), wr);
					return false;
				}
			}
			//修改 配置项 Key
			...
			bool SetConfigItemKey(const Tstr& _Config, const Tstr& _ConfigItem_Key, const Tstr& _ConfigItem_NewKey)
			{
				...
				if (tempConfig != this->ConfigMap.end()) {
					if (this->ShowManageLog) {
						lgc(_T("SetConfigItemKey: 修改配置项 Key"));
						lgc(_T("  配置[") + _Config + _T("]"));
					}
					auto tempConfigItem = tempConfig->second.find(_ConfigItem_Key); //对应配置
					if (tempConfigItem != tempConfig->second.end()) {
						...
						if (tempConfig->second.erase(_ConfigItem_Key)) {
							...
							if (this->ShowManageLog) {
								lgc(_T("SetConfigItemKey: 修改后..."), ts);
								lgc(_T("    配置项 Key[") + _ConfigItem_NewKey + _T("]"));
							}
						}
						else {
							lgc(_T("SetConfigItemKey: 删除旧配置项 Key 失败! 添加新配置项 Key 操作被跳过!"), wr);
							return false;
						}
					}
					...
					lgc(_T("SetConfigItemKey: 没有找到对应配置项!"), wr);
					return false;
				}
			}
			//修改 配置
			...
			bool SetConfig(const Tstr& _Config, const Tstr& _NewConfig)
			{
				...
				if (tempConfig != this->ConfigMap.end()) {
					if (this->ShowManageLog) {
						lgc(_T("SetConfig: 修改配置"), ts);
						lgc(_T("  配置[") + _Config + _T("]"), ts);
					}
					...
					if (this->ConfigMap.erase(_Config)) { //删除旧配置
						...
						if (this->ShowManageLog) {
							lgc(_T("SetConfig: 修改配置[") + _Config + _T("]成功!"), ts);
						}
					}
					else {
						lgc(_T("SetConfig: 删除旧配置[") + _Config + _T("]失败!"), wr);
						return false;
					}
			...
			//创建文件
			template<class T = bool>
			bool CreateFile(const Tstr& _FileEncode = _T("UTF-8"))
			{
				if (!WriteFile(this->ConfigFilePath, this->TextCache, _FileEncode)) {
					lgc(_T("CreateFile: 创建文件失败!"), wr);
					return false;
				}

				lgc(_T("CreateFile: 创建文件成功!"), ts);
				return true;
			}
			template<class T = bool>
			bool CreateFile(const Tstr& _NewFilePath, const Tstr& _FileEncode = _T("UTF-8"))
			{
		...
		public:
			...
			*/
			template<class T = bool>
			bool WriteConfigFile(bool _IsSureWrite = true, const Tstr& _FileEncode = _T("UTF-8"))
			{
				if (!this->FormatText) {
					if (!_IsSureWrite) {
						...
						if (this->Format(tempWriteConfig_Vec)) {
							if (!WriteFile(this->ConfigFilePath, tempWriteConfig_Vec, _FileEncode)) {
								lgc(_T("WriteConfigFile: 可能没有对应的文件, 或文件正在被使用!"), wr);
								return false;
							}
								...
								if (tempWriteConfig_Vec.size() > 2)
								{
									...
									for (auto tempBegin = tempWriteConfig_Vec.begin(); tempBegin != tempWriteConfig_Vec.end(); tempBegin++)
									{
										...
										if (tempOtherChar != this->OtherCharMap.end()) {
											lgc(_T("  写入其他字符: ") + tempOtherChar->second, lf);
										}
										lgc(_T("  写入字符: ") + *tempBegin, lf);

										tempOtherCharLine_Count++;
									}
								}

								lgc(_T("WriteConfigFile: 配置写入文件成功!"), ts);
							}
							...
							lgc(_T("WriteConfigFile: 格式化配置文本失败!"), wr);
							return false;
						}
					}
					else { //强制写入
					...
					if (this->ShowManageLog) {
						lgc(_T("WriteConfigFile: 已格式化配置文本!"), ts);
					}

					if (!WriteFile(this->ConfigFilePath, this->TextCache, _FileEncode)) {
						lgcr(_T("WriteConfigFile: 可能没有对应的文件, 或文件正在被使用!"), wr);
						return false;
					}
			...
			//写入文件: 文本
			template<class T = bool>
			bool WriteTextFile(std::vector<Tstr>& _Text, const Tstr& _FileEncode = _T("UTF-8"))
			{
				if (!WriteFile(this->ConfigFilePath, _Text, _FileEncode)) {
					lgc(_T("WriteTextFile: 可能没有对应的文件, 或文件正在被使用!"), wr);
					return false;
				}
				else {
					if (this->ShowManageLog) {
						...
						{
							if (this->ShowManageLog) {
								lgc(_T("  写入字符: ") + *tempBegin);
							}
						}

					}

					lgc(_T("WriteTextFile: 文本写入文件成功!"), ts);
					return true;
				}
			}
			//读取文件: 配置
			template<class T = bool>
			bool ReadConfigFile(const Tstr& _FileEncode = _T("UTF-8"))
			{
				//读取配置文件
				if (!ReadFile(this->ConfigFilePath, this->TextCache, _FileEncode)) {
					lgc(_T("ReadConfigFile: 可能没有对应的文件, 或文件正在被使用!"), wr);
					return false;
				}
				...
				if (this->ShowManageLog) {
					...
					{
						lgc(_T("  读取字符: ") + *tempBegin, lf);

					}

				}

				lgc(_T("ReadConfigFile: 文件读取配置成功!"), ts);
				return true;
			}
			//读取文件: 文本
			template<class T = bool>
			bool ReadConfigFile(std::vector<Tstr>& _Text, const Tstr& _FileEncode = _T("UTF-8"))
			{//读取配置文件
			...
			bool Format(std::vector<Tstr>& _FormatMap)
			{
				...
				if (!this->ConfigMap.empty())
				{
					...
					for (auto tempConfig = this->ConfigMap.begin(); tempConfig != this->ConfigMap.end(); tempConfig++)
					{
						...
						//配置
						auto tempConfigText = _T("[") + tempConfig->first + _T("]");
						if (this->ShowManageLog) {
							lgc(tempConfigText);
						}
						if (FirstLineText) { //第一个配置不需要
							_FormatMap.push_back(tempConfigText + _T("\n")); //带换行符
							FirstLineText = false;
						}
						else { //每一个配置中间使用换行隔开
							_FormatMap.push_back(_T("\n"));
							_FormatMap.push_back(tempConfigText + _T("\n")); //带换行符
						}
						...
						{
							auto tempText = tempConfigItem->first + _T("=") + tempConfigItem->second;
							if (this->ShowManageLog) {
								lgc(tempText);
							}
							if (tempConfigItem->second.find('\n') == Tstr::npos)
							{
								_FormatMap.push_back(tempText + _T("\n")); //带换行符
							}
							else //有换行符
							{
			...
			*/
			...
			bool Analyze()
			{
				...
				if (!(this->TextCache.size() < 2)) {
							...
							if (!NewConfig.empty()) { //有配置
								...
								if (this->ConfigItemTextManage(tempAnalyzeText, AnalyzeLaterConfigItem_Key, AnalyzeLaterConfigItem_Value)) {
									if (AnalyzeLaterConfigItem_Key.empty()) { //Key为空
										lgc(_T("Analyze: 传入的配置项[Key]格式有误!"), wr);
										lgc(_T("    Key: ") + AnalyzeLaterConfigItem_Key, wr);
									}
									if (AnalyzeLaterConfigItem_Value.empty()) { //Value为空
										lgc(_T("Analyze: 传入的配置项[Value]格式有误!"), wr);
										lgc(_T("    Value: ") + AnalyzeLaterConfigItem_Value, wr);
									}

									AnalyzeLaterConfigItem.insert(std::make_pair(AnalyzeLaterConfigItem_Key, AnalyzeLaterConfigItem_Value)); //保存到 配置项map
									if (this->ShowManageLog) {
										lgc(_T("    配置项 Key  [") + AnalyzeLaterConfigItem_Key + _T("]"));
										lgc(_T("    配置项 Value[") + AnalyzeLaterConfigItem_Value + _T("]"));
									}
								}
							}
							else {
					...
					if (this->ShowManageLog) {
						lgc(_T("Analyze: 解析完成!"), ts);
					}

					return true;
				}
				else {
					lgc(_T("Analyze: 传入的配置项不足以形成格式!"), wr);
					ConfigFormatSample();
					return false;
				}
			}
			...
			ConfigStringFormat AnalyzeFormat(Tstr& _AnalyzeConfig)
			{
				if (!_AnalyzeConfig.empty()) {
					...
					if (tempConfigChar == '[') { //配置
						...
						if (this->ShowManageLog) {
							lgc(_T("AnalyzeFormat: OtherChar[") + _AnalyzeConfig + _T("]"), wr);
						}
					...
					lgc(_T("AnalyzeFormat: 传入为空字符串!"), wr);
					return ConfigStringFormat::OtherChar;
				}
			}
		...
		private:
			...
			{
				//格式是否正确
				size_t tempConfigFormat_Right = _Config.find(_T(']')); //右括号
				if (tempConfigFormat_Right == Tstr::npos) {
					lgc(_T("ConfigTextManage: 格式错误!"), wr);
					lgc(_T("ConfigTextManage: 配置文本[") + _Config + _T("]"), wr);
			...
			{
				//格式是否正确
				size_t tempEqualSign = _ConfigItem.find(_T('=')); //找到 "=" 号的下标
				if (tempEqualSign == Tstr::npos) {
					lgc(_T("ConfigItemTextManage: 格式错误!"), wr);
					lgc(_T("ConfigItemTextManage: 配置项[") + _ConfigItem + _T("]"), wr);
					return false;
				}
				...
				//删除文本中的换行符 '\n'
#ifdef _WINDOWS
				if (*(_ConfigItem.end() - 1) == _T('\n') || *(_ConfigItem.end() - 1) == _T('\r')) { //Windows 找到 '\n' || '\r'
					_ConfigItem.erase(_ConfigItem.end() - 1, _ConfigItem.end());
					if (this->ShowManageLog) {
						lgc(_T("ConfigItemTextManage: 删除换行"), ts);
						lgc(_T("    Text[") + _ConfigItem + _T("]"));
					}
				}
#else
		...
		public:
			...
			{
				lgc(_T("ConfigFormatSample: 配置格式 示例开始..."), ts);
				lgc(_T("  ConfigFileName: cfg.ini"));
				lgc(_T("  ConfigItem: \n    config1(key1=value1, key2=value2)\n    config2(key3=value3)\n"));
				lgc(_T("  cfg.ini: 下面是实际的文本内容"));
				lgc(_T("  [config1]"));
				lgc(_T("  key1=value1"));
				lgc(_T("  key2=value2"));
				lgc(_T("  [config2]"));
			...
			{
				this->OtherCharMap.insert(std::make_pair(0, _T("//配置格式 示例\n// [ping-baidu]\n\
